...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for event in orm['rca.EventItem'].objects.all():
            external_link_pk = None
            if event.external_link:
                external_link, created = orm['rca.EventItemExternalLink'].objects.get_or_create(page=event, link=event.external_link, defaults=dict(text=event.external_link_text))
                external_link_pk = external_link.pk

            # Update revisions
            for revision in event.revisions.all():
                # Decode json
                revision_data = json.loads(revision.content_json)

                # Add external link
                if revision_data['external_link']:
                    revision_data['external_links'] = [{
                        'pk': external_link_pk,
                        'page': event.pk,
                        'link': revision_data['external_link'],
                        'text': revision_data['external_link_text'],
                    }]
