...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for pressrelease in orm['rca.PressRelease'].objects.all():
            area_pk = None
            if pressrelease.area:
                area, created = orm['rca.PressReleaseArea'].objects.get_or_create(page=pressrelease, area=pressrelease.area)
                area_pk = area.pk

            # Update revisions
            for revision in pressrelease.revisions.all():
                # Decode json
                revision_data = json.loads(revision.content_json)

                # Add area
                if revision_data['area']:
                    revision_data['areas'] = [{
                        'pk': area_pk,
                        'page': pressrelease.pk,
                        'area': revision_data['area'],
                    }]
