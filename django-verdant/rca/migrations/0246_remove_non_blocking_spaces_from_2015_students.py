...
class Migration(DataMigration):
    ...
    def remove_non_blocking_spaces_from_content(self, content):
        return content.replace('\xa0', ' ').replace('&nbsp;', ' ')

    def remove_non_blocking_spaces_from_page(self, student):
        student.show_work_description = self.remove_non_blocking_spaces_from_content(student.show_work_description)
        student.mphil_statement = self.remove_non_blocking_spaces_from_content(student.mphil_statement)
        student.phd_statement = self.remove_non_blocking_spaces_from_content(student.phd_statement)

    def remove_non_blocking_spaces_from_json(self, student):
        student['show_work_description'] = self.remove_non_blocking_spaces_from_content(student['show_work_description'])
        student['mphil_statement'] = self.remove_non_blocking_spaces_from_content(student['mphil_statement'])
        student['phd_statement'] = self.remove_non_blocking_spaces_from_content(student['phd_statement'])

    def forwards(self, orm):
        q = models.Q(ma_in_show=True, ma_graduation_year=2015) | models.Q(mphil_in_show=True, mphil_graduation_year=2015) | models.Q(phd_in_show=True, phd_graduation_year=2015)

        for page in orm['rca.NewStudentPage'].objects.filter(live=True).filter(q):
            self.remove_non_blocking_spaces_from_page(page)
            page.save()

            revision = page.revisions.order_by('-created_at')[0]
            revision_data = json.loads(revision.content_json)
            self.remove_non_blocking_spaces_from_json(revision_data)
