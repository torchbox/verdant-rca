...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for programme in orm['rca.ProgrammePage'].objects.all():
            carousel_item, created = orm['rca.ProgrammePageFacilitiesCarouselItem'].objects.get_or_create(
                page=programme,
                defaults=dict(
                    facilities_text=programme.facilities_text,
                    facilities_image=programme.facilities_image,
                    facilities_link=programme.facilities_link,
                )
            )
            carousel_item_pk = carousel_item.pk

            if created:
                # Update revisions
                for revision in programme.revisions.all():
                    # Decode json
                    revision_data = json.loads(revision.content_json)

                    # Add carousel item
                    revision_data['facilities_carousel_items'] = [{
                        'pk': carousel_item_pk,
                        'page': programme.pk,
                        'facilities_text': revision_data['facilities_text'],
                        'facilities_image': revision_data['facilities_image'],
                        'facilities_link': revision_data['facilities_link'],
                    }]
