...
class Migration(DataMigration):
    MIGRATE_FROM = ['research', 'knowledgeexchange']
    MIGRATE_TO = 'research-knowledgeexchange'

    def migrate_area(self, orm, model, field):
        pages = model.objects.filter(**{field + '__in': self.MIGRATE_FROM})
    ...
    def migrate_area_child(self, orm, model, relation, child_model, field):
        child_objects = child_model.objects.filter(**{field + '__in': self.MIGRATE_FROM})
    ...
    def forwards(self, orm):
        self.migrate_area(orm, orm.NewsItem, 'area')
        self.migrate_area(orm, orm.PressRelease, 'area')
        self.migrate_area(orm, orm.EventItem, 'area')
        self.migrate_area_child(orm, orm.EventItem, 'related_areas', orm.EventItemRelatedArea, 'area')
        self.migrate_area(orm, orm.StandardIndex, 'news_carousel_area')
        self.migrate_area(orm, orm.StandardIndex, 'events_feed_area')
        self.migrate_area_child(orm, orm.StaffPage, 'roles', orm.StaffPageRole, 'area')
        self.migrate_area(orm, orm.RcaNowPage, 'area')
        self.migrate_area(orm, orm.ResearchInnovationPage, 'news_carousel_area')
