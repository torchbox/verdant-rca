...
class Migration(DataMigration):
    ...
    def set_url_path(self, orm, node, path):
        node.url_path = path
        node.save()
        for child in orm['core.Page'].objects.filter(depth=node.depth + 1, path__startswith=node.path):
            self.set_url_path(orm, child, path + child.slug + '/')

    def forwards(self, orm):
        "Write your forwards methods here."
        # Note: Remember to use orm['appname.ModelName'] rather than "from appname.models..."

        for root in orm['core.Page'].objects.filter(depth=1):
            self.set_url_path(orm, root, '/')
