...
from verdant.verdantcore.models import Page, Orderable
from verdant.verdantcore.fields import RichTextField
...
from verdant.verdantimages.edit_handlers import ImageChooserPanel
from verdant.verdantimages.models import AbstractImage, AbstractRendition
from verdant.verdantdocs.edit_handlers import DocumentChooserPanel
from verdant.verdantsnippets.edit_handlers import SnippetChooserPanel
from verdant.verdantsnippets.models import register_snippet
...
class CarouselItemFields(models.Model):
    ...
    link_page = models.ForeignKey('verdantcore.Page', on_delete=models.SET_NULL, related_name='+', null=True, blank=True)
...
class Advert(models.Model):
    page = models.ForeignKey('verdantcore.Page', related_name='adverts', null=True, blank=True)
...
class AdvertPlacement(models.Model):
    page = ParentalKey('verdantcore.Page', related_name='advert_placements')
...
class CustomContentModuleBlock(Orderable):
    ...
    link = models.ForeignKey('verdantcore.Page', null=True, blank=True, on_delete=models.SET_NULL, related_name='+')
...
class CustomeContentModulePlacement(models.Model):
    page = ParentalKey('verdantcore.Page', related_name='custom_content_module_placements')
...
class ReusableTextSnippetPlacement(models.Model):
    page = ParentalKey('verdantcore.Page', related_name='reusable_text_snippet_placements')
...
class ContactSnippetPlacement(models.Model):
    page = ParentalKey('verdantcore.Page', related_name='contact_snippet_placements')
...
class SchoolPageRelatedLink(Orderable):
    ...
    link = models.ForeignKey('verdantcore.Page', null=True, blank=True, related_name='+')
...
class SchoolPage(Page, SocialFields):
    ...
    head_of_school_link = models.ForeignKey('verdantcore.Page', null=True, blank=True, on_delete=models.SET_NULL, related_name='+')
    ...
    head_of_research_link = models.ForeignKey('verdantcore.Page', null=True, blank=True, on_delete=models.SET_NULL, related_name='+')
...
class ProgrammePage(Page, SocialFields):
    ...
    head_of_programme_link = models.ForeignKey('verdantcore.Page', null=True, blank=True, on_delete=models.SET_NULL, related_name='+')
    ...
    facilities_link = models.ForeignKey('verdantcore.Page', null=True, blank=True, on_delete=models.SET_NULL, related_name='+')
        ...
        results = NewsItem.objects.extra(
            select={'score': """
                ...
                + (
                    ...
                    WHERE rca_newsitemrelatedprogramme.page_id=verdantcore_page.id
                        AND rca_newsitemrelatedprogramme.programme IN %s
                ) * 10
                + (
                    SELECT COUNT(*) FROM rca_newsitemrelatedschool
                    WHERE rca_newsitemrelatedschool.page_id=verdantcore_page.id
...
class EventItemSpeaker(Orderable):
    ...
    link_page = models.ForeignKey('verdantcore.Page', null=True, blank=True, on_delete=models.SET_NULL, related_name='+')
...
class FutureEventItemManager(models.Manager):
    def get_query_set(self):
        return super(FutureEventItemManager, self).get_query_set().extra(
            where=["verdantcore_page.id IN (SELECT DISTINCT page_id FROM rca_eventitemdatestimes WHERE date_from >= %s OR date_to >= %s)"],
...
class FutureNotCurrentEventItemManager(models.Manager):
    def get_query_set(self):
        return super(FutureNotCurrentEventItemManager, self).get_query_set().extra(
            where=["verdantcore_page.id IN (SELECT DISTINCT page_id FROM rca_eventitemdatestimes WHERE date_from >= %s)"],
            params=[date.today()]
        )
...
class PastEventItemManager(models.Manager):
    def get_query_set(self):
        return super(PastEventItemManager, self).get_query_set().extra(
            where=["verdantcore_page.id NOT IN (SELECT DISTINCT page_id FROM rca_eventitemdatestimes WHERE date_from >= %s OR date_to >= %s)"],
            params=[date.today(), date.today()]
        )
...
class StandardIndex(Page, SocialFields):
    intro = RichTextField(blank=True)
    intro_link = models.ForeignKey('verdantcore.Page', null=True, blank=True, on_delete=models.SET_NULL, related_name='+')
    strapline = models.CharField(max_length=255, blank=True)
    body = RichTextField(blank=True)
...
class ResearchItemCreator(Orderable):
    ...
    person = models.ForeignKey('verdantcore.Page', null=True, blank=True, related_name='+', help_text="Choose an existing person's page, or enter a name manually below (which will not be linked).")
...
class DonationPage(Page, SocialFields):
    redirect_to_when_done = models.ForeignKey('verdantcore.Page', null=True, blank=False, on_delete=models.PROTECT, related_name='+')
